...
static void *vendedor(void *ptr) {
  ...
  while (!start) pthread_cond_wait(&c, &m);
  ...
  for (;;) {
    ...
    if (vendo(precio, nom, comprador)) {
      ...
      while (comp->status != CHECK) pthread_cond_wait(&comp->c, &comp->m);
...
static void *comprador(void *ptr) {
  ...
  for (int i = 0; i < N; i++) {
    ...
    if (precio > 0) {
      ...
      comp->status = CHECK;
      pthread_cond_broadcast(&comp->c);
      while (comp->status != OK) pthread_cond_wait(&comp->c, &comp->m);
      pthread_mutex_unlock(&comp->m);
    } else
      usleep(1);
  }
