...
.while_begin:           #     // del ciclo para mayor eficiencia
    ...
    lw a0, 0(t0)                    # Cargamos la primera palabra p[0]
    li a2, 32                       # Espacio en ASCII
    li a5, 0                        # 0 := procesamos la primera, 1 := procesamos la segunda.

    .count:
        li a3, 0                    # int res = 0;
        lbu a4, 0(a0)               # char c = *s;
        j .for                      # Primer for (;;)
    ...
    .for:
        beq a4, zero, .break_for    # if (c == '\0') break;
        beq a4, a2, .continue       # if (c == ' ') { ... }

    .word_handle:
        beq a4, a2, .break_word     # if (c == ' ' || c == '\0') break;
        beq a4, zero, .break_word    
        addi a0, a0, 1              # s++;
        lbu a4, 0(a0)               # c = *s;
        j .word_handle              # Vuelve al ciclo (es un for (;;))

    .break_word:
        addi a3, a3, 1              # res++;
        j .for                      # Vuelve al for (;;) del inicio.

    .second_word:
        mv t1, a3                   # Guardamos el resultado
        addi a5, a5, 1              # Indicamos que estamos procesando la segunda palabra
        lw a0, 4(t0)                # Cargamos la segunda palabra p[1]
        j .count                    # Saltamos al inicio para procesar la segunda palabra

    .break_for:
        beq a5, zero, .second_word  # Si a5 = 0, entonces estamos en la primera palabra aún
        sub t1, t1, a3              # Guardamos la diferencia en t1
